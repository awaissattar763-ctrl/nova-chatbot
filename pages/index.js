...
export default function Home() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const reply = data.reply || "Kuch issue hua, dobara try karo.";
      const final = [...updated, { role: "assistant", content: reply, time: Date.now(), sender: "Nova", reactions: [] }];
      setMessages(final);
      setNewMsgIdx(final.length - 1);
    } catch {
      const err = [...updated, { role: "assistant", content: "⚠️ Connection error. Please try again.", time: Date.now(), sender: "Nova", reactions: [] }];
      setMessages(err);
    } finally {
      setLoading(false);
